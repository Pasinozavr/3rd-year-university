...
float fit(float t){return -18*t;}
float u(float x, float t)
{
	if(x-a*t>0.5){return 0;}
	if(x+a*t<0.1){return 0;}
	if(x-a*t<0.1 && x+a*t>0.5){return ((0.5-0.1)/(2*a))*fit(t);}
	if(0.1<x-a*t && x-a*t<0.5 && x+a*t>0.5){return fit(t)*((0.5-(x-a*t))/(2*a));}
	if(0.1<x+a*t&&x+a*t<0.5&&x-a*t<0.1){return (x+a*t-0.1)*fit(t)/(2*a);}
	if(x-a*t>0.1 && x+a*t<0.5){return t*fit(t);}
}
